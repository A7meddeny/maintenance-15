...
class Repository {
    ...
    suspend fun rentalFollowEmployees(): Outcome<List<FollowEmp>> {
        val o = callGet("rentals.php", mapOf("action" to "follow_employees"))
        return map(o) { gson.fromJson(dataArray(it), Array<FollowEmp>::class.java).toList() }
    }

    suspend fun rentalCreate(
        carId: Int, renterName: String, renterPhone: String,
        days: Int, contractNo: String, followUserId: Int
    ): Outcome<Unit> {
        val o = callPost("rentals.php", mapOf(
            "action" to "create", "car_id" to carId.toString(),
            "renter_name" to renterName, "renter_phone" to renterPhone,
            "days" to days.toString(), "contract_no" to contractNo,
            "follow_user_id" to followUserId.toString()
        ))
        return map(o) { }
    }

    suspend fun rentalMyAssignments(): Outcome<List<Rental>> {
        val o = callGet("rentals.php", mapOf("action" to "my_assignments"))
        return map(o) { gson.fromJson(dataArray(it), Array<Rental>::class.java).toList() }
    }
    ...
    suspend fun rentalSubmitInspection(
        rentalId: Int, kind: String, checklistJson: String, bodyPointsJson: String,
        km: String, note: String, contractNo: String, hasIssues: Boolean,
        photos: List<UploadFile>
    ): Outcome<Unit> {
        val parts = photos.mapIndexedNotNull { i, uf -> part("photo_$i", uf) }
        val o = callPost("rentals.php", mapOf(
            "action" to "submit_inspection", "rental_id" to rentalId.toString(),
            "kind" to kind, "checklist" to checklistJson, "body_points" to bodyPointsJson,
            "km" to km, "note" to note, "contract_no" to contractNo,
            "has_issues" to (if (hasIssues) "1" else "0")
        ), parts)
        return map(o) { }
    }
